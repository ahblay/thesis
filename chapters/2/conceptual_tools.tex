...
A screenshot of the visualizer is shown in Figure \ref{fig:visualizer}. It functions as follows:

A user enters the parameters of their problem (e.g. they wish to examine the grid $[17] \times [23]$ under 3-neighbor percolation, and so select ``Grid" from the dropdown menu and choose 17 rows, 23 columns, 1 layer, and a threshold of 3 neighbors), and then clicks submit. This renders a grid of gray vertices. The user then selects those vertices that they wish to infect and, when satisfied, clicks ``Run percolation." This allows the user to step through the phases of infection. 

Let us examine in detail the options available in this process. The control panel features the following:
