...
\begin{defn}
Let $a_1, \dots, a_d$ be integers such that $a_1 \geq \dots \geq a_d \geq 1$. We define the tuple $(a_1, \dots, a_d)$ as the grid graph $\prod_{i=1}^d [a_i]$. Furthermore, we refer to the smallest value $a_d$ as the \emph{thickness} of $(a_1, \dots, a_d)$.
\end{defn}
\noindent We remark that this notation is the same as that used for the coordinates of a vertex, and make explicit note of this distinction when necessary. 
