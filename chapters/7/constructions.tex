...
Let $X_1, \dots, X_{k_1}$ be the repeated components of $H$ in the $x$-direction, and $Y_1, \dots, Y_{k_2}$ be the repeated components of $H$ in the $y$-direction. Denote by $X \times Y$ the union of these components. Let $A_t \subseteq V(H)$ be the set of infectious vertices in $H$ at time $t$, and suppose that all $X_i$ and all $Y_i$ contain the same pattern of infected vertices (see Figure \ref{fig:3x12x2_unfolded_lethal}). We show that $A_0$ is lethal and perfect.

Consider the initial infection $A_0$ of $H$ as shown in figure \ref{fig:11x20x2_unfolded_lethal}. Observe that $A_0$ infects all vertices of $X\times Y$ by Proposition \ref{prop:immune_regions}. We show that the remaining healthy vertices of $H$ become infected. Consider re-folding $H$, and note that the two cells marked with an ``X" in $H$ represent the same cell in $G$. This is enough to infect the remaining regions of $H$, and by Corollary \ref{cor:walls}, $A_0$ is lethal on $G$. 
...
To prove that $A_0$ is perfect, observe that each $X_i \times Y_j$ block contains exactly 12 infected vertices. Of the remaining vertices $H \setminus X \times Y$, 20 are infected. Therefore, $|A_0| = 12(k_1)(k_2) + 20$. The surface area bound for $G=(6k_1+2,6k_2+5,2)$ is given by
$$\frac{(6k_1+2)(6k_2 + 5) + (6k_2+5)(2) + (2)(6k_1+2)}{3} = \frac{36(k_1)(k_2) +42k_1 +24k_2 + 24}{3} = 12k_1k_2 +14k_1+8k_2+8.$$
Since these two values are equal, $A_0^{k-1}$ is tight and lethal, and therefore perfect.
\end{proof}
